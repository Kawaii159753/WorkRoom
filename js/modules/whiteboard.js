 ...
 */
            ...
            function renderPostitLibrary() {
                ...
                grid.innerHTML = items.map(function (item) {
                    ...
                    return '<div class="postit-card' + (item.pinned ? ' pinned' : '') + '" data-color="' + escapeHtml(item.color || 'yellow') + '" role="button" tabindex="0" onclick="openPostit(\'' + escapeHandlerString(item.id) + '\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openPostit(\'' + escapeHandlerString(item.id) + '\')}" oncontextmenu="openPostitContext(event,\'' + escapeHandlerString(item.id) + '\')" aria-label="เปิดอ่าน ' + escapeHtml(item.title) + '">'
                        + (item.pinned ? '<span class="postit-pin" aria-label="'+(currentLang==='en'?'Pinned':'ปักหมุดแล้ว')+'">📌</span>' : '')
                        + (canEdit ? '<button class="postit-card-close" onclick="deletePostit(event,\'' + escapeHandlerString(item.id) + '\')" title="ลบโปสต์อิท" aria-label="ลบโปสต์อิท ' + escapeHtml(item.title) + '">×</button>' : '')
