 ...
 */
            ...
            renderTaskDetail = function () {
                ...
                var links = (row.links || []).map(function (link, i) { var href = safeExternalUrl(link.url); return href ? '<div class="workflow-link"><span>↗</span><a href="' + escapeHtml(href) + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(link.label || link.url) + '</a>' + (canEdit ? '<button onclick="removeWorkflowLink(' + i + ')" aria-label="ลบลิงก์">×</button>' : '') + '</div>' : ''; }).join('') || '<div class="task-empty-copy compact">ยังไม่ได้เชื่อมไฟล์ เอกสาร หรือหน้าวาด</div>';
            ...
            renderTaskDetail = function () {
                ...
                var links=row.links.map(function(link,i){var href=safeExternalUrl(link.url);return href?'<div class="workflow-link"><span>↗</span><a href="'+escapeHtml(href)+'" target="_blank" rel="noopener noreferrer">'+escapeHtml(link.label||link.url)+'</a>'+(canEdit?'<button onclick="removeWorkflowLink('+i+')" aria-label="'+t.deleteLink+'">×</button>':'')+'</div>':'';}).join('')||'<div class="task-empty-copy compact">'+t.noLinks+'</div>';
            ...
            function postitFullBlockElement(block,index,readonly){
                ...
                if(block.type==='embed'){var raw=String(block.url||block.content||'').trim();var candidate=/^https?:\/\//i.test(raw)?raw:'https://'+raw;var href=safeExternalUrl(candidate);wrapper.innerHTML=href?'<div class="postit-full-link"><span>↗</span><a href="'+escapeHtml(href)+'" target="_blank" rel="noopener noreferrer">'+escapeHtml(raw||'ลิงก์')+'</a></div>':'<div class="postit-full-placeholder">'+(currentLang==='en'?'Invalid link':'ลิงก์ไม่ถูกต้อง')+'</div>';return finishPostitFullBlock(wrapper,block,index,readonly);}
